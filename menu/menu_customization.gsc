...
TypeWriterFXText(text, hud)
{
    ...
    while(true)
    {
        if(isDefined(hud))
        {
            level.colorR = RandomIntRange(160, 255); // Rango para el componente rojo (valores m?s altos)
            level.colorG = RandomIntRange(160, 255); // Rango para el componente verde (valores m?s altos)
            level.colorB = RandomIntRange(160, 255); // Rango para el componente azul (valores m?s altos)
...
LevelRGBFade(target_color, time)
{
    ...
    while (true)
    {
        ...
        t = elapsed / time; // Calcula el factor de interpolaci?n
        color = LerpVector(start_color, target_color, t); // Interpolaci?n lineal del color
